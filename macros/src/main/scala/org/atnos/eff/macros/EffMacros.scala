      ...
      case Some(q"$_ trait ${tpname: TypeName}[..$_] extends { ..$earlydefns } with ..$parents { $self => ..$stats }") =>
        ...
        stats.foreach {
          ...
          case d @ DefDef(_, _, _, _, rt, EmptyTree) =>
            if (!isReturnTypeOfTypeAlias(rt))
              c.abort(d.pos, s"Abstract def needs to have return type ${typeAlias.name}[...], otherwise, make it non-abstract.")
          case _ => // no issue
        }

        val absValsDefsOps: Seq[ValOrDefDef] = stats.collect { case m @ DefDef(_, _, _, _, ExpectedReturnType(_), EmptyTree) =>
          m
        }
        ...
        val genCaseClassesAndObjADT =
          absValsDefsOps.collect { case q"..$_ def $tname[..$tparams](...$paramss): ${AppliedTypeTree(_, returnType)} = $expr" =>
        ...
        val sideEffectTrait = {
          val methodsToBeImpl: Seq[DefDef] = absValsDefsOps.map { case q"..$mods def $name[..$tparams](...$paramss): Eff[$_, $returnType]" =>
        ...
        val translatorFactoryTraits = (1 until 10).map { numOutEffects =>
          ...
          val effectImplicitParams = (1 to numOutEffects)
            .map(i => ValDef(Modifiers(Flag.IMPLICIT), replacementParamName(i), tq"${TypeName(s"R$i")} MemberIn U", EmptyTree))
            .toList
          ...
          q"""
              ...
              TypeDef(
                ...
                TypeName(s"R$i"),
                List(TypeDef(Modifiers(Flag.PARAM), typeNames.WILDCARD, List(), TypeBoundsTree(TypeTree(), TypeTree()))),
