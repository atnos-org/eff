...
final class TwitterFutureOps[R, A](val e: Eff[R, A]) extends AnyVal {
  ...
  def runAsync(implicit pool: FuturePool, timer: Timer, ev: Eff[R, A] =:= Eff[Fx.fx1[TwitterTimedFuture], A]): Future[A] =
    TwitterFutureInterpretation.runAsync(e)

  def runSequential(implicit pool: FuturePool, timer: Timer, ev: Eff[R, A] =:= Eff[Fx.fx1[TwitterTimedFuture], A]): Future[A] =
