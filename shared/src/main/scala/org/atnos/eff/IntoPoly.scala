  ...
  implicit def intoNil[R]: IntoPoly[NoFx, R] =
    new IntoPoly[NoFx, R] {
      def apply[A](e: Eff[NoFx, A]) =
        e match {
          case Pure(a, last) => pure[R, A](a).plusLast(apply(last.run))
  ...
  implicit def intoAppendL2L[T1[_], T2[_], R]: IntoPoly[FxAppend[Fx1[T2], R], FxAppend[Fx2[T1, T2], R]] =
    new IntoPoly[FxAppend[Fx1[T2], R], FxAppend[Fx2[T1, T2], R]] {
      def apply[A](e: Eff[FxAppend[Fx1[T2], R], A]): Eff[FxAppend[Fx2[T1, T2], R], A] =
        e match {
          case Pure(a, last) =>
            EffMonad[FxAppend[Fx2[T1, T2], R]].pure(a).plusLast(apply(last.run))

          case Impure(u@UnionAppendR(r), c, l) =>
            Impure[FxAppend[Fx2[T1, T2], R], u.X, A](UnionAppendR(r), Arrs.singleton(x => effInto(c(x))), Last.eff(apply(l.run)))

          case Impure(u@UnionAppendL(Union1(tx)), c, l) =>
            Impure[FxAppend[Fx2[T1, T2], R], u.X, A](UnionAppendL(Union2R(tx)), Arrs.singleton(x => effInto(c(x))), Last.eff(apply(l.run)))
          ...
          case ImpureAp(unions, continuation, l) =>
            ImpureAp[FxAppend[Fx2[T1, T2], R], unions.X, A](
              unions.into(new UnionInto[FxAppend[Fx1[T2], R], FxAppend[Fx2[T1, T2], R]] {
                ...
                }}), Arrs.singleton(x => apply(continuation(x))), Last.eff(apply(l.run)))
        }
    }
    ...
    new IntoPoly[FxAppend[Fx1[T1], R], FxAppend[Fx2[T1, T2], R]] {
      def apply[A](e: Eff[FxAppend[Fx1[T1], R], A]): Eff[FxAppend[Fx2[T1, T2], R], A] =
        e match {
          ...
          case Impure(u@UnionAppendL(Union1(tx)), c, l) =>
            Impure[FxAppend[Fx2[T1, T2], R], u.X, A](UnionAppendL(Union2L(tx)), Arrs.singleton(x => effInto(c(x))), Last.eff(apply(l.run)))
  ...
  implicit def intoAppendL3L[T1[_], T2[_], T3[_], R]: IntoPoly[FxAppend[Fx2[T2, T3], R], FxAppend[Fx3[T1, T2, T3], R]] =
    new IntoPoly[FxAppend[Fx2[T2, T3], R], FxAppend[Fx3[T1, T2, T3], R]] {
      def apply[A](e: Eff[FxAppend[Fx2[T2, T3], R], A]): Eff[FxAppend[Fx3[T1, T2, T3], R], A] =
        e match {
          case Pure(a, last) =>
            EffMonad[FxAppend[Fx3[T1, T2, T3], R]].pure(a).plusLast(apply(last.run))

          case Impure(u@UnionAppendR(r), c, l) =>
            Impure[FxAppend[Fx3[T1, T2, T3], R], u.X, A](UnionAppendR(r), Arrs.singleton(x => effInto(c(x))), Last.eff(apply(l.run)))

          case Impure(u@UnionAppendL(Union2L(tx)), c, l) =>
            Impure[FxAppend[Fx3[T1, T2, T3], R], u.X, A](UnionAppendL(Union3M(tx)), Arrs.singleton(x => effInto(c(x))), Last.eff(apply(l.run)))

          case Impure(u@UnionAppendL(Union2R(tx)), c, l) =>
            Impure[FxAppend[Fx3[T1, T2, T3], R], u.X, A](UnionAppendL(Union3R(tx)), Arrs.singleton(x => effInto(c(x))), Last.eff(apply(l.run)))
    ...
    new IntoPoly[FxAppend[Fx2[T1, T3], R], FxAppend[Fx3[T1, T2, T3], R]] {
      def apply[A](e: Eff[FxAppend[Fx2[T1, T3], R], A]): Eff[FxAppend[Fx3[T1, T2, T3], R], A] =
        e match {
          ...
          case Impure(u@UnionAppendL(Union2L(tx)), c, l) =>
            Impure[FxAppend[Fx3[T1, T2, T3], R], u.X, A](UnionAppendL(Union3L(tx)), Arrs.singleton(x => effInto(c(x))), Last.eff(apply(l.run)))
  ...
  implicit def intoAppendL1[T[_], R]: IntoPoly[R, FxAppend[Fx1[T], R]] =
    new IntoPoly[R, FxAppend[Fx1[T], R]] {
      def apply[A](e: Eff[R, A]): Eff[FxAppend[Fx1[T], R], A] =
        e match {
          case Pure(a, last) =>
            EffMonad[FxAppend[Fx1[T], R]].pure(a).plusLast(apply(last.run))

          case Impure(u, c, l) =>
            Impure[FxAppend[Fx1[T], R], u.X, A](UnionAppendR(u), Arrs.singleton(x => effInto(c(x))), Last.eff(apply(l.run)))
  ...
  implicit def into[T[_], R, U, S](implicit
    ...
    new IntoPoly[R, U] {
      def apply[A](e: Eff[R, A]): Eff[U, A] =
        e match {
          case Pure(a, last) =>
            EffMonad[U].pure(a).plusLast(apply(last.run))

          case Impure(u, c, l) =>
            t.project(u) match {
              case Right(tx) => Impure[U, u.X, A](m.inject(tx), Arrs.singleton(x => effInto[R, U, A](c(x))), Last.eff(apply(l.run)))
              case Left(s)   => recurse(Impure[S, s.X, s.X](s, Arrs.singleton(x => pure(x)))).flatMap((x: s.X) => effInto[R, U, A](c(x))).plusLast(apply(l.run))
            }
  ...
  implicit def intoMember[T[_], R, U](implicit m: Member.Aux[T, R, U]): IntoPoly[U, R] = new IntoPoly[U, R] {
    def apply[A](e: Eff[U, A]): Eff[R, A] =
      e match {
        ...
        case Impure(u, c, l) =>
          Impure(m.accept(u), Arrs.singleton((x: u.X) => intoMember.apply(c(x))), Last.eff(apply(l.run)))

        case ImpureAp(unions, c, l) =>
          ImpureAp(unions.into(new UnionInto[U, R] { def apply[X](u: Union[U, X]) = m.accept(u) }),
            Arrs.singleton((xs: List[Any]) => intoMember.apply(c(xs))), Last.eff(apply(l.run)))
      }
  }
}
