...
trait state {
  ...
  implicit class StateEffectOps[R, A](e: Eff[R, A]) {
    ...
    def lensState[BR, U, T, S](getter: S => T, setter: (S, T) => S)(
      implicit m1: Member.Aux[State[T, *], R, U],
               m2: Member.Aux[State[S, *], BR, U]): Eff[BR, A] =
      StateInterpretation.lensState[R, BR, U, T, S, A](e, getter, setter)

    def intoState[BR, U1, T, S, U2](getter: S => T, setter: (S, T) => S)(
      implicit m1: Member.Aux[State[T, *], R, U1],
